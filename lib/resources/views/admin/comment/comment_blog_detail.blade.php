            ...
            <div class="row mb-2">
                <div class="col-sm-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="{{url('admin/commentblog')}}">Bình luận bài viết</a></li>
                        <li class="breadcrumb-item active">Chi tiết bình luận bài viết</li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        ...
                        <!-- /.card-header -->
                        <div class="card-body">
                            <form action="{{url('admin/commentblog/delete')}}" method="POST" >
                            @csrf
                                @can('xoa binh luan')
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @if($row->status == 1)
                                                <textarea class="form-control reply_comment_{{$row->id}}" rows="3"></textarea> <br>
                                                <button class="btn btn-xs btn-primary send_reply_comment" data-comment_id="{{$row->id}}" data-blog_id="{{$row->blog_id}}">Trả lời</button>
                                                @endif
                                            </td>
                                            <td>{{date('d/m/Y H:i',strtotime($row->created_at))}}</td>
                                            <td>
                                                @if($row->status == 1)
                                                    <input type="button" data-comment_status="0" data-comment_id="{{$row->id}}" id="{{$row->blog_id}}" class="btn btn-danger btn-xs comment_allow" value="Bỏ duyệt">
                                                @else
                                                    <input type="button" data-comment_status="1" data-comment_id="{{$row->id}}" id="{{$row->blog_id}}" class="btn btn-success btn-xs comment_allow" value="Duyệt">
                                                @endif
                                            </td>
                                        </tr>
                                        @endforeach
